                 ...
                 {currentRecords.length === 0 ? <p className="text-center text-gray-500 py-4">Nenhum registo encontrado.</p> : (
                    ...
                    {activeSubCategory === 'Combustível' && (
                         <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4 items-center">
                            <div className="h-64"><h4 className="text-md font-semibold text-center text-gray-700 mb-2">Distribuição por Município</h4><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={Object.entries(currentRecords.reduce((acc, r) => ({...acc, [r.municipio]: (acc[r.municipio] || 0) + Number(r.quantidadeRecebida)}), {} as Record<string, number>)).map(([name, value]) => ({name, value}))} dataKey="value" nameKey="name" cx="40%" cy="50%" outerRadius={80} fill="#8884d8">{Object.keys(currentRecords).map((_, index) => <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />)}</Pie><Tooltip formatter={(value) => `${value} L`} /><Legend layout="vertical" verticalAlign="middle" align="right" /></PieChart></ResponsiveContainer></div>
                            {renderTable()}
                        </div>
                    )}
